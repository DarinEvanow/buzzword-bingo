defmodule BuzzwordBingo.GameServer do
  @moduledoc """
  ...
  def init({game_name, size}) do
    buzzwords = BuzzwordBingo.BuzzwordCache.get_buzzwords()

    game =
      case :ets.lookup(:games_table, game_name) do
        [] ->
          game = BuzzwordBingo.Game.new(buzzwords, size)
  ...
  def handle_call({:mark, phrase, player}, _from, game) do
    new_game = BuzzwordBingo.Game.mark(game, phrase, player)
  ...
  defp my_game_name do
    Registry.keys(BuzzwordBingo.GameRegistry, self()) |> List.first
  end
end
